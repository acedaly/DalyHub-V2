...
-- real D1 with the development provider (`test/kernel/grounded-ai.test.ts`),
-- which is the entire argument for GROUND-00: a platform whose every layer is
-- mocked is precisely the platform whose first real request fails.
--
-- ## Why a table rebuild
--
-- SQLite cannot alter a CHECK constraint in place. The twelve-step procedure is
-- the documented way, and it is safe here for reasons worth stating rather than
-- assuming:
--
