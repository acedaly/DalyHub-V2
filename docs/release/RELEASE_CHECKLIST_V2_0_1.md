...
| **`db:production:list` / `db:production:apply` could not work at all** — found by review on this PR, and a genuine pre-existing defect rather than one this release introduced | `scripts/production-d1.mjs` wrote its temporary Wrangler config to the OS temp directory (deliberately, so no real identifier lands in a tracked file) while setting `migrations_dir` to the RELATIVE `"migrations"`. Wrangler resolves a relative `migrations_dir` against the **config file's** directory, so it pointed at `/tmp/dalyhub-d1-XXXX/migrations` — never present — and exited 1 with `No migrations present at /tmp/…/migrations`. Both documented migration commands were therefore broken, **and** the new preflight's fail-closed migrations check would have refused every production deploy with no override covering it. Now an absolute repository path. | `scripts/production-d1.mjs`; `test/unit/deploy/production-d1.test.ts` — resolves the emitted value exactly as Wrangler does, asserts the real `0001`–`0025` sequence is on the other end, and asserts the old relative value would NOT have resolved. Verified empirically against the pinned Wrangler 4.112.0 before and after (exit 1 vs the full list, exit 0); an injected fake runner cannot expose a path bug, which is why the test resolves a real path instead. |
| No automated backups | `.github/workflows/production-backup.yml`: daily 16:30 UTC + `workflow_dispatch`, exports via the audited `production-d1.mjs` wrapper, dated commit-stamped artifact with `metadata.json`, 30-day retention, fails on a missing/empty/schema-less export, least-privilege (`contents: read`), secrets via the `production` GitHub environment, nothing printed, nothing committed. **Not restore** — SET-02 remains V2.1. | The workflow file; documented in [`DEPLOYMENT.md → Automated production backups`](../development/DEPLOYMENT.md#automated-production-backups-to-github-v201-encrypted-in-v21--audit-11) |
