...
| 7b | `pnpm run deploy:production:release-check -- --acknowledge-pending-migrations` and **read the `ROLLBACK BOUNDARY` lines** | ⏳ Added 2026-09-16 (DEPLOY-01). §1.2 established "no rollback after `0050`" for THIS release by hand; the preflight now derives it from [`migration-ledger.json`](../development/migration-ledger.json) for every release, and names each pending migration that removes something along with what it removes. Run it here so the window you are about to open is on the record before you open it, not after |
| 8 | `pnpm run db:production:apply` | §1.1 — **required for this release**. §1.2 — **the point of no rollback**: production is on a schema the running Worker cannot serve from here until step 9 lands. The boundary is no longer a claim in this document — `db:compat` derives it, and CI fails when the ledger stops matching the migrations |
