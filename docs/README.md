...
| [`development/SHARED_SEARCH.md`](development/SHARED_SEARCH.md) | The Shared Search system (DS-08): the model/orchestrator/UI layers, the registry-driven provider contract and the `SearchResultTarget` navigation refinement, deterministic ranking & grouping, bounds & safety, incremental search (debounce/abort/stale), Drawer opening, accessibility & modal reuse, the server composition, the Today fixture provider, the **NOTES-03 Notes full-content provider** (title/body/headings/tags, match sources, readable excerpts) and its D1-native indexing strategy + trade-offs, and the development demonstration. |
...
| [`development/TODAY_DASHBOARD.md`](development/TODAY_DASHBOARD.md) | The Today dashboard (TODAY-01…TODAY-08): where it lives and how it composes, the canonical Task Drawer, the Waiting model, the planning model (the scheduled date as the owner's commitment, kept distinct from the due date), the keyboard workflow, the phone shell and Quick Capture, the command-centre landing, what is deliberately NOT built, its disclosed deviations, and **its relationship to the completed Tasks collection (TASKS-03)** — one definition of "today", verified rather than assumed. |
