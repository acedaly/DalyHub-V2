...
| [`development/DIARY_MODULE.md`](development/DIARY_MODULE.md) | The Diary architecture & kernel foundation (DIARY-01A, backend-only): the Interstitial Journal philosophy (capture first, organise later), the `diary` entity type + additive `diary_entry_details` slice, chronology-first `occurred_at` (Memory-Mode backdating), the open registration-based entry-type vocabulary, the authoritative `DiaryRepository` (reserved atomic capture + edit + the bounded/cursor-paginated Timeline read model with day/month grouping), the Activity-vs-Diary boundary, relationships via EntityLinks, and the future roadmap it enables. |
