...
| [`development/VIEWS_MODULE.md`](development/VIEWS_MODULE.md) | The Views module (X-02, ADR-080): cross-module saved views. The ONE declarative view configuration generalised from the Tasks contract — shared versus module-specific dimensions and the rule that a dimension a record type cannot answer removes that record type rather than widening the query; one saved-view system with a `kind` (migration `0035`, additive) shared by Tasks and cross-module views; the bounded per-scope query engine and its asserted statement cost; the REVIEW-03 boundary and health-movement integration; the built-in views; module visibility, archive and delete semantics; and the scopes deliberately deferred. |
