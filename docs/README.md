...
| [`development/ACTIVITY_TIMELINE.md`](development/ACTIVITY_TIMELINE.md) | The Shared Timeline & Activity Feed (DS-05): one renderer for both scopes, the presentation view-model boundary, registering event descriptors, the unknown-type fallback, wiring a route, DS-07 filtering & DS-03 drawer reuse, ordering/grouping/dates, virtualisation, accessibility, the real product adopters (the task record + the **project record Activity tab, PROJ-04**), and the development demonstration. |
...
| [`development/PROJECTS_MODULE.md`](development/PROJECTS_MODULE.md) | The Projects module (PROJ-01 + PROJ-02 + PROJ-04): data ownership (the spine authority + the read-only `ProjectRepository` projection, `getRollup` as the progress source of truth, `listProjectTasks`), the routes, the shared composition (collection · overview · Tasks/Key-links/**Activity** tabs · create/rename forms), the re-homed shared task-record surface (ADR-033), the Today integration, the **derived project-health signal (PROJ-02, ADR-035)**, the **project Activity tab (PROJ-04, ADR-036)**, testing, and what remains for PROJ-03/05/06. |
