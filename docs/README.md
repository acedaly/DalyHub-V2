...
| [`development/PROJECTS_MODULE.md`](development/PROJECTS_MODULE.md) | The Projects module (PROJ-01 + PROJ-02): data ownership (the spine authority + the read-only `ProjectRepository` projection, `getRollup` as the progress source of truth, `listProjectTasks`), the routes, the shared composition (collection · overview · Tasks/Key-links tabs · create/rename forms), the re-homed shared task-record surface (ADR-033), the Today integration, the **derived project-health signal (PROJ-02, ADR-035)**, testing, and what remains for PROJ-03–06. |
