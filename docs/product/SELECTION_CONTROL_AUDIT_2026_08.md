...
| **Obligation meter unit "Not set"** ([`ObligationForm`](../../app/shared/obligations/ObligationForm.tsx)) | "Not set" | An obligation with no meter target is a *date-only* obligation — a real shape, not a half-filled one. The form was `AssetObligationForm` until V2.10 LIFE-02 converged the Assets tab and Life Admin onto one form. |
