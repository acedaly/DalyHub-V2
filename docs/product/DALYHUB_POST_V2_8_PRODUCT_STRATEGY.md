...
1. **A kernel Activity window read** — count-by-type-per-bucket and list-in-window over the existing `(workspace_id, type, occurred_at, id)` index — replacing five bespoke adapter reads with one (DEBT-238). One grouped statement per question, flat in workspace size, the way ADR-079 d2 already does it for one caller.
...
| subject entity (optional) | yes | nullable FK to `entities` — an Asset, a Person, a Project, an Area; the Assets module's obligation lens filters on it |
...
| **Transactions**: date, amount (minor units, signed), account, payee (normalised + display), memo, category, `import_id`, `source_fingerprint`; rows, **not** entities; soft delete | attachments on a transaction (V2.11 makes it possible; V2.12 links them where the owner does it by hand) |
...
| **Deduplication**: `(account, date, amount, normalised payee)` plus the import fingerprint; a suspected duplicate is *shown and skipped*, never silently merged | fuzzy cross-account reconciliation |
...
- *Idempotent imports*: file hash + row fingerprint; the same file twice yields zero new rows and says so.
...
- *Query/index requirements*: the V2.9 history layer's `sum-by-group-per-bucket` over `finance_transactions` — the same statement shape as completions-per-bucket.
...
| **Transaction** (V2.12) | new table `finance_transactions`; **not an entity** | mutable rows; the **import** is the immutable, audited unit | one Activity per applied import (counts); no per-row Activity | new collection, paged in the archive | `(workspace, account, occurred_on, id)`, `(workspace, category, occurred_on)`, `(workspace, source_fingerprint)` unique per account | `settled_by` → Obligation by id; attachments by link through the account's record until a transaction drawer offers it | payee under explicit query; amounts never excerpted | composite | low per table; **volume** is the new property — first paged collection in the archive |
...
| **Import idempotency** | n/a | n/a | file hash + row fingerprint; a repeated import yields zero rows and says so; the import ledger survives export/restore |
