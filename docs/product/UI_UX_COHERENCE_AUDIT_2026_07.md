...
| **UXA-07** | All records | **No overflow (⋯) menu is rendered anywhere.** `RecordHeader` renders only inline primary/secondary buttons, and the Card's `overflowAction` slot is unused by all seven modules — so the conventional home for secondary/destructive actions doesn't exist. | `RecordHeader.tsx:95-111` (no overflow render); `Card.tsx` `overflowAction` present but passed by no module | P1 | Structural | There is nowhere consistent to put Archive/Delete/Restore/Duplicate; every module invents a different placement (UXA-05/06/08). | Shared | Implement the documented Record Header overflow slot (DESIGN_SYSTEM.md:82) and adopt the Card `overflowAction` slot; house lifecycle actions there. | **PX-04** |
...
| **UXA-13** | Today | **Today's primary action is a forked bespoke button** (`dh-today__primary` "Quick capture", not a `DrawerTrigger`), and **Quick Capture is inert** — submitting announces "not saved". | `TodayDashboard.tsx:844-852`; TODAY_DASHBOARD.md:441-446 | P2 | Daily | The home screen's headline action does nothing and looks different from every other module's create button. | Adoption + Domain | Wire Quick Capture to real task creation; use the shared primary-action pattern. | **PX-06** / existing TODAY backlog |
...
| 2 | **Record Header overflow (⋯) menu** + adopt Card `overflowAction` | A consistent home for secondary/destructive actions | All records | Implement documented `RecordHeader` overflow slot | S–M | Low | none | Yes |
...
| 6 | **Wire shared `EmptyState` into Today sections; make Quick Capture real** | Calm home never dead-ends; headline action works | Today | Adopt EmptyState; reuse create pattern | S–M | Low–Med | task-create wiring | Yes |
...
- **Scope:** Add `PriorityIndicator` + `UrgencyChip` shared components (§6); render on the shared Card
  ...
  Tasks search payload to carry priority.
...
### PR-2 — `PX-04a` Record Header overflow menu (structural)
...
### PR-3 — `PX-04b` Unified lifecycle & destructive-action consistency
...
- **Merge deps:** PR-2. **Model:** **Claude Opus** (lifecycle UX, precondition messaging, cross-module).
...
- **Scope:** Wire shared `EmptyState` into Today sections; converge create labels/count nouns/empty copy
  ...
  tab label; single Diary primary entry point per viewport. (Quick-Capture wiring and fixture-row
  retirement can ride here or in the existing TODAY backlog.)
