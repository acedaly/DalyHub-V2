...
- **TODAY-08 note.** The command-centre landing retired the calendar/notes/timeline *fixtures* and wired the surface to real reads, but the Today **search provider** is out of scope for that item and is still fixture-backed (unchanged). A minor related limitation surfaced: the Notes widget presents the most-recent of a bounded first page because `entities.list` orders ascending by `(createdAt, id)` with no reverse option — correct for an early workspace, approximate beyond the page bound; a dedicated recent-notes read (or a `newest` order option) resolves it alongside real Notes search.
...
- **Related roadmap item.** [TASKS-02](../roadmap/ROADMAP_V2.md#-tasks-02--shared-task-signal-presentation).
