...
- **Current issue.** AREA-02's canonical `/goals/:goalId` record shows a single bounded first page (50) of Projects advancing the Goal, with an honest "more Projects exist" note when a `nextCursor` is present — matching AREA-01's Area Goals/Projects tabs, not the Project record's Tasks tab's interactive `useFetcher`-based "Load more" affordance. A Goal with more than 50 linked Projects can only reach the remainder by other navigation (e.g. the `/projects` collection filtered manually), not from the Goal record itself.
- **Assessment.** The EXACT Project-contribution progress (`GoalRepository.getGoalProjectContribution`) is unaffected — it already reads the complete, unbounded fact set independent of this display page. This is a display-page ergonomics gap, not a correctness or progress-accuracy gap, and mirrors the SAME accepted shape AREA-01 shipped for its own Goals/Projects tabs.
- **Desired future state.** Add a `GET /goals/:goalId/projects` bounded resource route and an interactive "Load more" affordance on the Projects tab, following the exact pattern `ProjectTasksTab`/`/projects/:projectId/tasks` already established (keyset pagination, no navigation, drawer-state-preserving).
- **Related roadmap item.** [AREA-02](../roadmap/ROADMAP_V2.md#-area-02--goals).

### ☐ DEBT-23 — Alignment priority ordering does not span pages, and no alignment history is kept — P3
- **Current issue.** AREA-03's `/goals` Alignment collection fetches one bounded, creation-order-keyset page of Goals, computes each Goal's alignment from the COMPLETE underlying facts (always exactly correct regardless of pagination), then sorts only THAT page by state precedence (`neglected` → `active` → `unreachable` → `no_structure` → `completed`) for display. A neglected Goal on page 2 is never promoted above an active Goal on page 1 — the "most in need of a look" ordering is per-page, not workspace-wide. Separately, alignment is recomputed live on every read (ADR-040's "derived, never cached" ethos, matching PROJ-02/AREA-01/AREA-02); no history of a Goal's alignment over time is stored, so "was this Goal neglected last week too" cannot be answered from the Alignment view itself.
- **Assessment.** DalyHub is single-owner and most workspaces hold well under one page (50) of Goals, so the cross-page ordering gap has low real-world impact; each Goal's OWN classification is always exactly correct, which is the property that actually matters for trust. Alignment history is a genuinely different feature (a time series, not a snapshot) — deferring it avoids inventing a second persisted analytics model without evidenced need.
- **Desired future state.** If a workspace's Goal count regularly exceeds one page, consider a bounded "most neglected first" aggregate query (mirroring the momentum/health facts-gathering pattern) instead of a per-page sort. Alignment trend/history, if ever wanted, is a `REVIEW-03`-shaped concern (a derived reflection surface), not a change to this read.
- **Related roadmap item.** [AREA-03](../roadmap/ROADMAP_V2.md#-area-03--alignment-view); [REVIEW-03](../roadmap/ROADMAP_V2.md#-review-03--insights--alignment) (a possible future consumer).
