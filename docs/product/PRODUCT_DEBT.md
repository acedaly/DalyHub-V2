...
### ☐ DEBT-16 — Minimal task-detail model (no rich workflow status or Waiting) — P3
- **Current issue.** [TODAY-02](../roadmap/ROADMAP_V2.md#-today-02--task-drawer) ([ADR-028](../decisions/ARCHITECTURE_DECISIONS.md#adr-028-task-drawer-persistence-and-composition--the-additive-task-detail-slice)) added only the smallest honest task-detail slice: a two-value workflow `status` (`todo`/`in_progress`, with "done" derived from spine completion), a three-value `priority`, due/scheduled dates and a Markdown description. A richer workflow status (planned/active/on-hold/cancelled) and a Waiting/blocking field are **not** modelled.
- **Desired future state.** A full Tasks module models the richer status and (via [TODAY-03](../roadmap/ROADMAP_V2.md#-today-03--waiting)) the Waiting/blocking relationship, extending the same `task_details`/EntityLink foundation rather than a parallel store.
- **Related roadmap item.** [TODAY-03](../roadmap/ROADMAP_V2.md#-today-03--waiting) and a future Tasks module.
