...
- **Rich-text editors — Tiptap / ProseMirror / Lexical / Milkdown / Monaco (mostly 🟢 MIT).** Authoring tools, not a render/storage foundation. **Rejected for FND-08** (the render/sanitise pipeline) and still rejected as a *storage* model — they persist a rich-text/proprietary document, violating the Markdown-source strategy. **CodeMirror 6** was subsequently adopted (NOTES-05) as an *authoring surface only* — see its evaluation below — because, unlike the WYSIWYG editors, its document is plain text (the Markdown source stays canonical).

### Reuse evaluation — CodeMirror 6 (NOTES-05)

- [x] Problem is a commodity worth reusing — a live, in-place Markdown *authoring* surface with stable cursor/undo/IME (not a differentiator; the differentiator is DalyHub's Markdown-source + FND-08 architecture, which we own).
- [x] Checked this file for existing notes — the earlier "not adopted / revisit when a Notes item genuinely needs rich authoring while still saving Markdown source" note; NOTES-05 is exactly that revisit.
- [x] Licence read for the exact installed versions → **MIT** (Allowed by default) for every direct package and the entire transitive `@codemirror/*` / `@lezer/*` / `crelt` / `style-mod` / `w3c-keyname` tree.
- [x] Transitive tree licence-checked — no copyleft/prohibited pulled in (all MIT).
- [x] Maintenance health acceptable — CodeMirror 6 is actively maintained (the editor core behind Obsidian and many others), typed, tree-shakeable, framework-agnostic.
- [x] No known unresolved critical CVEs.
- [x] Footprint acceptable — a few hundred KB, **code-split and lazy-loaded onto the note-editor route only** (constructed client-side; the server renders an accessible `<textarea>` fallback), so it never enters the initial bundle or the SSR path.
- [x] Fits our stack, accessibility bar and Design System — SSR/Workers-safe (no top-level DOM access), a labelled `textbox`, and styled entirely through DS-01 tokens.
- [x] No privacy-violating telemetry / phone-home.
- [x] Provenance recorded — pinned exact versions in `pnpm-lock.yaml`, `THIRD_PARTY_NOTICES.md`, and [ADR-044](../decisions/ARCHITECTURE_DECISIONS.md#adr-044-the-writing-first-live-markdown-editor--adopting-codemirror-6-as-an-authoring-surface-over-the-unchanged-fnd-08-source-and-render-pipeline).
- [x] Fits the one load-bearing rule — whatever we adopt must still save Markdown **source** through the FND-08 boundary. CodeMirror's document IS the source; it emits no HTML and never becomes the stored representation.

Decision: **Depend** — CodeMirror 6 as an authoring surface only, over the unchanged FND-08 source-and-render pipeline.
