...
| Obligation | — | `event_available` |
| Finance account | orange `#EF6C00` | `account_balance` |
...
**The accent space is full, and a type may BORROW rather than force a fourteenth
hue** (V2.12, [ADR-120](../decisions/ARCHITECTURE_DECISIONS.md) decision 2). Every
pair of accents is held to a **ΔE 10 (CIE76) perceptibility floor in both
appearances**, asserted in `test/unit/design/entity-accents.test.ts`. When V2.12
needed accents for two new types, sixteen candidate hues were probed against the
generated scheme and **three cleared the floor** — an orange, a dark green and a
rust. That is the design system reporting a real limit, not an obstacle to route
around.

So Finance took ONE. Orange, because green would read as *you're fine* and rust
as *you're not*, and **identity is never status**. `finance_transaction` — the
lightest entity in the product, with no record page, opening in the shared
Drawer — borrows the account's through a declared `BORROWED_IDENTITY` map, so the
borrow is a stated relationship a reader can find rather than a missing entry
that happens to fall back. The next type that needs a mark inherits the same
question, and the honest answers are: borrow one, or change the palette
deliberately.
...
**Purpose.** Make the SECOND task cheap. Every surface that holds a list of tasks lets you add the next one where you are already looking, with the same gesture: activate, type, `Enter`, the field clears and keeps focus, ready for another.
**Anatomy.** ONE component — [`InlineCaptureRow`](../../app/shared/task-record/InlineCaptureRow.tsx) — drawn as the row above (or at the foot of) the list it adds to: a `+` where a completion circle goes, a real labelled input, and an `Add` / `More options` pair that appears only when the row is engaged with. It is the same object at every width; only its host changes.
**Behaviour.** Posts to the canonical `/tasks/new` resource route, exactly as the capture Drawer does, so a task created here is created atomically, under a server-verified parent, with the same Activity trail. The same deterministic capture vocabulary (`p1`, `today`, `every Monday`, `#tag`) applies wherever the row is drawn. On a recoverable failure the typed text is never discarded, the error is shown inline and announced, and the field keeps focus.
**Rules.**
- **A caller says WHERE, never how it looks.** `destination` is the parent a task lands under; `defaults` is the classification the surface is already carrying (the priority a board column stands for, the sector a planning column names, the day a plan row sits under). Presentation is one stylesheet block.
...
**Anatomy.** Two containers — a PRIMARY column carrying the work and a RAIL carrying what needs attention (~360px at 1440) — each ONE tonal surface holding plain rows. No cards inside them, no nesting, no per-widget chrome. The split is **8 / 4 of twelve tracks**, stated in ONE place: the left cell of every working row is a list of titles and wants the measure, the right is a strip of times, a short run of facts or a doorway. Bands (the day's decision, the week's door, habits, the measures) take all twelve.
...
**Rules.** The rail holds only what the primary column does NOT show; anything appearing in both is one fact painted twice. **One seam, and it is declared once.** Every panel lands on the same vertical line at every width — the majors and minors must ALTERNATE in DOM order, because auto-placement pairs them and two adjacent majors leave four columns of hole and push the next panel down past it. A second block that re-states a span for one row is how a page grows a second gutter without anyone deciding to. **The day is the only thing raised.** Supporting regions keep their hairline and sit on the recessed ground: three ground values, in the order the ordinal ladder already defines them, rather than eight identical rectangles competing to be read first. There is no personalisation, no reordering and no hide/show — an arrangement the owner has to maintain is a second product to keep coherent. This pattern replaced the POLISH-02 hero/primary/secondary REGION model and its fourteen-widget catalogue, together with the at-a-glance stat rail those regions carried: a fixed track of six tiles is a good component and was the wrong answer, because on an ordinary day four of the six read `0`. See [The Today screen](#the-today-screen).
...
**Adoption.** DS-04 wired it into `/tasks` only. [TODAY-TASK-01](TODAY_TASK_01_ONE_TASK_ROW_2026_08.md) added **Today's plan** as its second caller and closed [DEBT-143](../product/PRODUCT_DEBT.md) with it — Today's private row is deleted, and the module declares no `.dh-today .dh-taskrow` structural override. PLAN-01 added Weekly Planning; V2.8 CONV-01 the Project record's Tasks tab (DEBT-175); V2.8 CONV-02 `/today/waiting` (DEBT-128), deleting the last generic Card that drew a Task and the UIX-01 override layer with it. **Every surface on which a Task can be acted on renders this row** (ADR-115 decision 2), enumerated by `test/unit/task-record/shared-row-consumers.test.ts`; **a reference to a Task — a search result, a `/views` row, a Meeting follow-up row, `NextActionLine` — is a link, never a row** (decision 3), asserted by `test/unit/task-record/reference-surfaces.test.tsx`. Exactly one component in `app/` is called `TaskRow`.
...
| `waiting` (optional) | `taskRowWaitingFact(item, nowMs)` — the row's ONE waiting fact (V2.8 CONV-02): the subject, the waiting instant, the SERVER's instant and RECALL-03's chase date, as inputs. The row formats them through the canonical waiting and date helpers and states the follow-up state in words (`data-follow-up-state` carries the machine value). Passed where the context holds the fact (`/today/waiting`; `/tasks` in a waiting/follow-up configuration); omitted elsewhere, and the row is unchanged. Never a second formatter, never a surface-drawn metadata run. |
...
It must not add a Today-shaped (or Projects-shaped) column, a per-surface stylesheet override of the grid, or a second optimistic applier — `applyTaskListItemPatch` is shared and generic over the record type for exactly that reason. **Shared anatomy is not every capability in every scope**: a capability whose precondition is absent on a surface stays absent through the row's own contract (a slot not passed, a prop set off) — drag without a destination, selection without a reason, "Plan for today" where the day would not draw the Task — never by forking the row. The per-surface table is in [`TASKS_MODULE.md`](../development/TASKS_MODULE.md#the-shared-row-and-what-a-scope-may-switch-off-v28-conv-01-2026-09-03).
...
**Controls (one per field type).** **Text** (single/multi-line, optional length readout, real `maxLength`, correct `autocomplete`). **Markdown source** (edits FND-08 source, preserved verbatim; a safe preview through the ONE `MarkdownContent` sink — no second parser, no new HTML sink; not the Notes editor). **Date** (unambiguous date-only + UTC datetime; see below). **Select** (single or multi; accessible editable combobox/listbox; client filter or async `onSearch`/`loading`; a stale/unavailable value is shown and labelled, never a crash). **Money** (V2.10 LIFE-02 — an amount and the ISO-4217 code it is in as ONE field, because [ADR-049](../decisions/ARCHITECTURE_DECISIONS.md) stores them together and converts nothing: an amount without its code is not a smaller fact, it is a wrong one. One label naming the AMOUNT input, one help line and one error slot shared by both controls, and the decimal keypad on every money input there is. It replaced four hand-rolled amount-plus-code pairs across three Asset forms, two of which put the code behind a disclosure two sections from the amount it labelled). **Signed money** (V2.12 — an amount whose DIRECTION is a control, never a typed minus sign. Finance signs every amount (positive in, negative out), and asking the owner to type the sign asks them to encode a convention the product already knows: a forgotten minus on an opening balance is wrong by twice the figure in every net-worth reading afterwards, and it looks exactly like a correct entry. So the form takes a magnitude and an explicit in/out — or *I owe this* / *I am in credit* on a liability account — and the SERVER composes the signed value, so one place decides what the sign means. The direction control is shown only where the sign is genuinely ambiguous: a credit card's opening balance, a hand-entered transaction). **Toggle group** (HABITS-01 — a small, already-familiar closed set, several of which may be on at once: real checkboxes in a labelled `role="group"`, painted as wrapping pill toggles, each with its own `accessibleLabel` so an abbreviated face is announced in full; the whole vocabulary is visible with nothing to open, and it wraps rather than scrolls on a phone. `SelectField multiple` remains the answer for a long or unfamiliar set — see [Selection controls](#forms)). **Tags** (controlled string collection; keyboard add/remove; normalisation, duplicate prevention, configurable limits; no tags database). **Boolean** (native checkbox or switch; clickable label; never colour-only). **Entity-link picker** (below).
...
**Grouping (PX-03, re-cut by V2.16 CONSOL-00).** A route's `meta.navGroup` (declared by the owning module) puts its row in one of six blocks. The blocks are the five **questions** the product answers, plus the machine: `do` **Do** (Today/Plan/Inbox/Upcoming/Tasks) · `organise` **Organise** (Projects/Goals/Areas/Habits/Notes/Diary/Meetings/People) · `deal-with` **Deal with** (Life Admin/Assets) · `money` **Money** (Finance) · `understand` **Understand** (Insight/Reports/Reviews/AI) · `system` (Views/Settings/Help/About, separated by position rather than a heading).

The group keys, their order, their headings and their accessible names live in one place — [`app/shared/shell/navigation-groups.ts`](../../app/shared/shell/navigation-groups.ts) — read by the rail, by the phone sheet and by the architecture test that asserts no module invents a seventh group. A module declares WHICH group it is in; it never declares what the group is CALLED, because a heading is a property of the frame's information architecture rather than of any one module.

**A group name is never a route.** There is no `/do`, no `/organise`, no `/deal-with`. Inventing a page to justify a label is how a taxonomy becomes a product surface nobody asked for.

**The grouping is semantic, not decorative.** PX-03 drew a decorative `<hr>` and, for two of its four groups, an `aria-hidden` caption — defensible while the groups were shapes ("daily", "more") that told a screen-reader user nothing. A heading that says **Money** is the shortest answer to *"where do I go for this?"*, so each block's destinations are now their own `<ul>` NAMED by its heading (`aria-labelledby`): a screen reader announces "Money, list, 1 item" once on entry rather than repeating the group per row. The `system` block, which has no visible heading, carries an `aria-label` instead — "separated by position" is a claim about pixels, and a screen reader has none. The rule between blocks stays decorative and `aria-hidden`; there is no landmark per group (the rail is already the Primary navigation region) and no accordion. Keyboard order equals visual order because the DOM order is the visual order.

**Ordering.** Within a block, `meta.navOrder` decides. Each block owns its own hundred (Do 1xx, Organise 2xx, Deal with 3xx, Money 4xx, Understand 5xx, system 9xx) so a new module lands somewhere on purpose — before V2.16, Finance and Views both claimed 210 and two rows of the rail were ordered by module-discovery glob order.
