...
**Roving-focus membership (`rovingTabIndex`).** For a keyboard-navigable collection (a surface where Arrow keys move a single focus across many cards — e.g. the Today task list, TODAY-05), the optional `rovingTabIndex` prop applies a `tabIndex` to EVERY interactive control the card owns (the open target, the selection checkbox and the action buttons), so the card joins a roving composite as ONE unit: `0` makes the card the collection's single tab stop, `-1` removes the whole card from the tab order. Undefined (the default) leaves natural tab behaviour unchanged, so every existing consumer is untouched. This is the accessible roving-`tabindex` pattern (like RecordTabs) rather than a `listbox`/`aria-activedescendant` composite — correct here because a card legitimately contains interactive children, which a listbox option may not.
