...
**Purpose.** ONE long-form editor for every surface that holds real prose — Notes, the Diary body and capture, Meeting agenda and notes, Reviews, a Task's description and (through [inline editing](#inline-editing-ds-16)) any record's long-form field. It should feel like Gmail or Docs: compact, embedded, familiar — not like a form panel bolted onto a page.
...
**Anatomy.** [`app/shared/inline-edit`](../../app/shared/inline-edit): one pure state machine (`inline-edit-model`) · one hook (`useInlineEdit`, which owns the async submission, the superseded-reply guard and focus) · one read affordance and editing frame (`InlineEditShell`) · one measurement hook (`useAnchoredAlignment`, which flips a popover to the inline-end edge rather than off the viewport) · four typed fields — `InlineTextField` (single-line, or `multiline` for plain text whose line breaks matter), `InlineMarkdownField` (on the [shared writing surface](#shared-writing-surface-edit-01)), `InlineSelectField` (an anchored WAI-ARIA menu button) and `InlineDateField` (an anchored `dialog` around a native date input).
**Behaviour.** A simple text field: activate the value, Enter saves, Escape cancels, blur saves. A multiline field (plain or Markdown): explicit **Save**/**Cancel** plus ⌘/Ctrl+Enter, blur does **not** save, and Escape cancels *only* while the draft is untouched. A select or date: a compact anchored popover with roving focus, Escape-to-close and focus returned to the trigger.
...
- **An anchored surface flips rather than overflowing.** The select menu and the date popover measure themselves on open and anchor from the inline-end edge when a start-anchored box would run past the viewport. Only the anchor changes — never the width — so nothing reflows, and no inline surface can produce a page-level horizontal scrollbar at 320px.
- **Short/simple text inline; Markdown only where Markdown is stored.** A three-word description does not get a 256px editor and a formatting toolbar for syntax the column cannot hold. Use `InlineTextField` for plain values (`multiline` when the line breaks are significant, as in a Goal's definition of done), `InlineMarkdownField` only for genuinely Markdown-backed long-form.
...
> A collection whose filters are executed **by a repository**, and *any* collection with **saved views**, uses the declarative view configuration in [`app/kernel/views`](../../app/kernel/views) ([ADR-080](../decisions/ARCHITECTURE_DECISIONS.md#adr-080-one-saved-view-system-two-kinds--the-tasks-declarative-configuration-generalised-into-a-cross-module-query-contract), [`VIEWS_MODULE.md`](../development/VIEWS_MODULE.md)).
>
