...
| D48 | **Six floating surfaces, one appearance, and a menu is not a picker** (DHDS-09) | M3 draws a menu, a dropdown and a dialog as three separately-specified components, and the product had grown six panels that each answered "border, corner, shadow" alone — including a form date picker that answered none of them and painted nothing at all. DalyHub publishes ONE surface (`.dh-floating`) with exactly two rungs, anchored and centred-modal, and ONE option row (`.dh-option`). What it deliberately does not share is the ARIA pattern: a menu, a listbox, a combobox and a dialog are different interactions, and the taxonomy exists so a surface declares which one it is rather than looking like the others |
| D49 | **A floating surface names a LAYER; it never picks a number** (DHDS-09) | The `--app-z-*` scale has eleven rungs, six of which nothing floats at, so a component reaching into it had to decide for itself whether a picker was a "dropdown" or an "overlay". Three had answered with a bare `z-index: 20`, and the two that chose `dropdown` (1000) rendered BEHIND the Drawer (1200) they were opened from. Seven `--dh-layer-*` roles, ordered by how much authority a surface has to speak over what is under it, and the anchored rung sits ABOVE the modal one because these surfaces are opened from inside it |
| D50 | **A row's SELECTED container is not its hover container** (DHDS-09) | Both were `--surface-muted` in every option list the product drew, so the current choice looked exactly like the one under the pointer and the two cancelled whenever they met — on a fifty-item Project menu, "which one is set?" was genuinely unanswerable. Selection takes the accent container and its own foreground, which is what `base.css` already said M3 does for a checked menu item; hover stays a quiet surface step. Both are reinforced by a check and a weight, so neither rests on colour |
| D51 | **Interaction does not require a pill** (DHDS-09) | The fastest way to make metadata clickable is to put a capsule round it, and doing that to a due date, a priority, a Project, a status, an Area and a Goal turns a calm list into a sticker album — which is the "no pill for ordinary metadata that works as text" prohibition, arrived at from the other direction. A contextual value is plain metadata at rest and grows a restrained container on hover AND on `:focus-visible`; the capsule is reserved for a genuine chip |
| D52 | **An editable value in a metadata RUN is quiet at rest; its affordances join the row reveal** (DHDS-10) | Every design system draws a select as a bordered box with a permanent chevron, and applying that to a Task's four metadata columns produces `[Tomorrow ▼] [DalyHub ▼] [P2 ▼] [Active ▼]` fifty times down a page — a beautifully composed list turned into a wall of form controls by an interaction improvement. DalyHub publishes ONE axis instead (`presentation="meta"`): the VALUE is always drawn, and the caret and the empty invitation become `.dh-action-reveal` elements inside the row's existing `data-dh-action-context`. So they fade in with the row's `⋯` button, on the product's one curve, from the product's one contract — and the page reads as information first and controls second. It is declared once, in `inline-edit.css`, precisely because the first version of it lived in `task-list.css` and every other surface that became editable would have inherited the loud default |
| D53 | **Quiet never costs a touch target** (DHDS-10) | Amends and completes D43. A `meta` field narrows to WCAG 2.2 §2.5.8's 24px minimum, and the narrowing is gated on `(hover: hover) and (pointer: fine)` exactly as M3X's `control-height-sm` rule above it is — a device must positively declare a fine pointer WITH hover to get the smaller target. Declaring 24px unconditionally is what a first cut did, and it took every metadata control on a phone from 44px to 24px; measured on the Asset record at 393px. A surface that genuinely cannot afford 44px of INK extends the hit area instead of shrinking the target, which is what the Task row's pseudo-element has always done |
| D54 | **A collection is draggable only when its order is the OWNER'S and is stored** (DHDS-11) | Every design system treats a list as reorderable and leaves it to the author to wire the persistence. Applied to DalyHub that produces a grip on a list sorted by due date: the drop appears to work and the next read undoes it, which is worse than no drag at all. DalyHub publishes a GATE instead — six questions, all of which must answer yes, chief among them that the change survives a reload. Two of the product's collections pass (`task_checklist_items.position`, `goal_milestones.position`). A Task's position in a list does NOT, because `task_details` deliberately carries no ranking column, and DHDS-11 declined to add a schema in order to give itself something to drag. The visible consequence is that DalyHub has far fewer grips than a product of its kind, and every one of them tells the truth |
| D55 | **A spatial move and a contextual choice are ONE mutation** (DHDS-11) | The obvious way to build drag is a drop handler that posts what the destination means. Do that beside an existing picker and the product has two implementations of one domain change — which drift, and whose validation, Activity and refusal wording diverge first. Dropping a Task on a Project and choosing that Project from its menu post the SAME `set_parent` intent to the same route; "Move up" in a checklist item's menu and dragging the step post the same `checklist_reorder`. The drag engine (`~/shared/drag`) owns no mutation at all — a source-level test fails the build on a `fetch` inside it — and the bucket-to-intent translation exists in exactly one file |
| D56 | **A destination that would change nothing is never drawn** (DHDS-11) | Material's dragged state layer (16%) describes what a dragged object looks like; nothing in it says which regions may light up. Left unspecified, every container under the pointer becomes a target and the page turns into a light show with a refusal at the end of it. DalyHub asks each destination whether it would accept THIS object — a Project a Task already belongs to answers no — and an unaccepting region registers nothing and paints nothing. Disclosure then has exactly two rungs: a candidate gains a dotted edge, the active one gains the accent. It is never possible to hover a lit target that then declines |
| D57 | **A row that the owner removed LEAVES; a row a filter removed simply goes** (DHDS-11) | The tempting rule is "animate every removal", and it is wrong twice over: a surface that KEEPS completed work would animate a departure that did not happen, and a changed filter would animate fifty. A departure is derived from the row having ACTUALLY left — an id the surface just mutated that is no longer in the loader's answer — so the animation cannot disagree with the event. Focus goes to the row that takes its place, never to `<body>`, which is the interaction decision DHDS-08 refused to guess at and this phase took |
