...
| Result rows | `application/table` + `overrides/table-head` | Name / Relative size / measure, right-aligned tabular figures, the full period visible beneath a short one | `role="rowheader"` on the name cell is React Aria's; the COLUMNS are DalyHub's |
