...
| --- | --- | --- | --- | --- |
| UIQ-011 | Person record summary | Medium | Eight equally-weighted tonal pill actions (Call … Copy phone) compete; Quick Actions says a curated 2–3 with the tail in overflow/palette. | **RESOLVED in #131** — Call and Email are primary (plus Message where a mobile exists), each rendered only where the data behind it exists; the four capture actions and both copy actions moved to the record header's shared overflow with the Person's capture context intact. Fixed as part of the record-screen convergence rather than alone, which is why it waited. |
