...
**Implementation note (2026-08 — the Today screen was replaced).** Today is no
longer a personalisable widget surface. It is the day: a greeting and date block
on the page canvas, a conditional chip row, and two tonal columns — the day's
work (overdue, meetings, due today) and an attention rail (Needs attention,
Continue working). Every chip, the progress indicator, every section and every
rail row is conditional on its own count, so a zero is never painted; overdue
work appears once as a chip and once as actionable rows, and is deliberately
absent from the rail. The widget catalogue, the personalisation model, the stat
rail, the Task Summary ring, Insights, Productivity and the search hero are gone.
The contract lives in [`DESIGN_SYSTEM.md` → The Today screen](DESIGN_SYSTEM.md#the-today-screen);
the implementation note is [`TODAY_DASHBOARD.md`](../development/TODAY_DASHBOARD.md).
The note below is retained as the historical record of what it replaced.

**Implementation note (TODAY-08 — command centre, superseded 2026-08).** Today is now DalyHub's primary landing page and working **command centre** ([ADR-045](../decisions/ARCHITECTURE_DECISIONS.md#adr-045-today-as-the-command-centre--the-personalisable-widget-landing-surface-over-real-cross-module-reads)): a **personalisable** set of widgets — Morning Brief · My day (the preserved planning/keyboard/swipe core) · Recent activity (the workspace-wide DS-05 feed) · Diary · Notes · Continue working · Areas · Goals · Focus (placeholder) · Insights · Capture — composed in the one `CollectionLayout`. The owner can hide/move/pin/collapse widgets and the arrangement is remembered per device. It reads REAL cross-module data (the calendar/notes/timeline fixtures are retired) and stays calm and enforced (no charts, no red badges; zero-count signals omitted; honest placeholders for weather/calendar/focus/Pomodoro). The reference sketch above remains the composition contract for the My-day planning region; the surrounding widgets are the same calm section idiom. See [`TODAY_DASHBOARD.md`](../development/TODAY_DASHBOARD.md).
