...
- **Keyboard shortcuts.** Built: full operability of every shipped component (tabs arrows/Home/End, drawer trap/Escape, reorder pick-up/move/drop, filter editor). Missing: any *global* accelerator. **Rule:** the Part IV vocabulary is reserved now; DS-09 implements; every action reachable by keyboard remains a merge gate.
