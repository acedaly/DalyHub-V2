...
| 16 | `markdown-editor.css` cannot be layered while CodeMirror injects its CSS at runtime | `app/app.css`, `app/styles/markdown-editor.css`, `~/shared/markdown-editor` | CodeMirror's `StyleModule` injects 72 unlayered rules, and unlayered beats every layer unconditionally — so layering DalyHub's editor stylesheet handed a third-party library absolute priority over the file that configures it. MEASURED: `.cm-content`'s `padding: 4px 0` beat DalyHub's inset, the editor's first line lost its left padding, and four E2E partitions went red on `editor-geometry.spec.ts`. The file is unlayered again, which is safe because **zero** of its 66 classes sit beside an Untitled utility in the markup. | None | Move the 49 `.cm-*` rules into CodeMirror's own `EditorView.theme()`, where they participate in the same StyleModule and precedence as the defaults they override. Then the file can be layered like everything else. | Medium | No |
