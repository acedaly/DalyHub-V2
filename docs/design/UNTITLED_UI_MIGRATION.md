...
| # | Item | Where | Why it remains | User impact | Next action | Priority | Blocks completion? |
| :-- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 12 | Eight legacy control stylesheets, 1,966 lines, in `dh-legacy` | `floating.css` (593), `ui.css` (536), `filters.css` (380), `pill.css` (125), `tooltip.css` (92), `overflow-menu.css` (91), `skeleton.css` (82), `progress.css` (67) | They sit in `dh-legacy` and therefore lose to every Untitled utility. None changed on screen when they started losing — but see item 12a: the 170-surface matrix under-sampled records and opened no overlay, so for three of them the diff proved nothing and a direct audit was needed instead. | None | Delete, file by file, and delete the file when its last rule goes. Start with `tooltip.css` and `progress.css`, which have no consumer at all. | Medium | No |
| 12a | `tooltip.css` (92 lines) and `progress.css` (67 lines) have NO CONSUMER | `~/shared/tooltip`, `~/shared/charts/ProgressRing` | Found while auditing item 12's evidence rather than by the cascade diff, which never rendered either. The shared Tooltip draws pure Untitled utilities (`rounded-lg bg-primary-*`, an `oklch` background) and `dh-tooltip` appears **zero** times in the rendered DOM. `progress.css` carries no `.dh-progress*` rules at all — UNTITLED-07 deleted them, the names survive as markup hooks — and its only live rules, `.dh-ring*`, serve `ProgressRing`, which is exported from the charts barrel and imported by nobody. | None | Delete both stylesheets. Decide separately whether `ProgressRing` itself should go: it is a data ring the charts barrel still exports, and CLAUDE.md rule 3's logic (a component with no product consumer) applies to it. | Medium | No |
