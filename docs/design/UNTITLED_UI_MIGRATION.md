...
| 6 | `application/progress-steps` for the guided Review's step rail | `~/shared/ui/untitled` (absent) | Still `access: "pro"` and not retrievable without an interactive login. **Public** component source IS retrievable through the CLI (proved by fetching `file-upload-base`), so this is a Pro gate specifically. **RE-CHECKED 2026-09-16 through the Untitled MCP connector, and the boundary is now named: catalogue access is not source access.** The connector reports `has_pro_access: true` and returns the component's metadata — `components:application`, 6 files, `access: "pro"`, `import { ProgressSteps } from '@/components/application/progress-steps/progress-steps'` — but the SOURCE comes from `npx untitledui@latest add progress-steps --yes`, and the tool's own instruction says that needs `npx untitledui@latest login` first. | None | Retrieve when interactive Pro CLI login exists; the current implementation is retained and documented, never faked | Low | No — blocked on access, not on design |
...
| # | Item | Where | Why it remains | User impact | Next action | Priority | Blocks completion? |
| :-- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 12 | **Seven** legacy control stylesheets, **1,899** lines, in `dh-legacy` | `floating.css` (593), `ui.css` (536), `filters.css` (380), `pill.css` (125), `tooltip.css` (92), `overflow-menu.css` (91), `skeleton.css` (82) | They sit in `dh-legacy` and therefore lose to every Untitled utility. None changed on screen when they started losing — but see item 12a: the 170-surface matrix under-sampled records and opened no overlay, so for three of them the diff proved nothing and a direct audit was needed instead. **`progress.css` is gone** (was 67 lines), with the component it painted. | None | Delete, file by file, and delete the file when its last rule goes. **`tooltip.css` is NOT the next one** — see the correction in 12a. | Medium | No |
| 12a | ~~`tooltip.css` and `progress.css` have NO CONSUMER~~ — **HALF WRONG, corrected 2026-09-16** | `~/shared/tooltip`, `~/shared/charts/ProgressRing` | **`progress.css` was right and is CLOSED**: it carried no `.dh-progress*` rules at all, its only live rules were `.dh-ring*`, and `ProgressRing` was exported from the charts barrel and imported by nobody. Component and stylesheet are deleted. **`tooltip.css` was WRONG and deleting it would have been a visible regression.** There are TWO tooltips: Untitled's vendored `base/tooltip` (used by Untitled's own `input`, `label`, `button-utility`, `table`, `section-label`, `command-input`, `avatar-add-button`), which is what the audit measured, and DalyHub's `~/shared/tooltip/Tooltip.tsx`, which **9 modules import** — `IconButton`, the editor toolbar, the overflow menu, the user menu, both top bars, the primary navigation. That one renders `className="dh-tooltip dh-motion-reveal"` with `.dh-tooltip__label` and `.dh-tooltip__shortcut` and carries **no Untitled utility**, so `tooltip.css` is its entire appearance. | None today; deleting `tooltip.css` would have left every tooltip in the shell chrome unstyled | The real finding is a SECOND generic tooltip beside Untitled's (CLAUDE.md rule 1). DalyHub's has behaviour Untitled's lacks — a keyboard-shortcut chip from the one shared notation formatter, so a tooltip reads `⌘B` on Apple and `Ctrl+B` elsewhere. Converging means giving Untitled's tooltip that slot, not deleting a stylesheet. | Medium | No |
| 13 | `forms.css` mixes generic field PAINT with field GEOMETRY — **and holds an accessibility floor** | `forms.css`, 1,120 lines | Demoting the whole file to `dh-legacy` correctly retired the paint and incorrectly retired the geometry with it. MEASURED: `.dh-combobox__input` lost the `padding-inline-end: 32px` that reserves room for its own trailing control, and its text would have run under a button 32px from the field's inner edge. The file keeps `dh-product` until it is split. | None | Split it: geometry stays in `dh-product`, paint moves to `dh-legacy` and then out. Item 1 above is the same job seen from the call-site end. **Whoever does it must keep the `@media (hover: none)` `min-block-size` in `dh-product`:** every control `inputClassName()` emits carries Tailwind's 38px `min-h-[var(--input-height)]` in `utilities`, and `ui.css`'s own 44px floor is ALREADY beaten by it — so this file's rule is the only thing holding WCAG 2.2 §2.5.8 on a coarse pointer. DEMONSTRATED: moving the file to `dh-legacy` drops a shared field to **38px** at 900px under `hover: none`. Gated by `e2e/touch-targets.spec.ts` → "shared controls on a coarse pointer, above the phone band", verified to fail on exactly that change. Raised by Codex review on #297: the predicted consequence does not occur today, the mechanism behind it does. | Medium | No |
...
| 16 | Eight rules cannot be layered while CodeMirror injects its CSS at runtime | `app/styles/markdown-editor-codemirror.css`, `~/shared/markdown-editor` | CodeMirror's `StyleModule` injects 72 unlayered rules, and unlayered beats every layer unconditionally — so layering DalyHub's editor stylesheet handed a third-party library absolute priority over the file that configures it. MEASURED: `.cm-content`'s `padding: 4px 0` beat DalyHub's inset, the editor's first line lost its left padding, and four E2E partitions went red on `editor-geometry.spec.ts`. The first fix un-layered the WHOLE editor stylesheet, which then outranked the five product surfaces that legitimately override it — `review-guide.css` stopped cancelling the compact density cap and axe reported a serious `scrollable-region-focusable` on the guided Review (p07, red on main). The exception is now **eight** rules in their own file, exactly those contesting a property CodeMirror's own sheet sets. It reached eight in two steps, and the second is the lesson: the first list was written out BY HAND from a probe that printed 60 of CodeMirror's 320 selector/property pairs, so `.cm-cursor`/`.cm-dropCursor`'s `border-left-color` and `.cm-placeholder`'s `color` stayed layered and lost. MEASURED: the text cursor was `rgb(0,0,0)` — **1.06:1** against the dark surface, against ~17:1 from the token — so in the dark appearance an owner could not see where they were typing, on every writing surface in the product; the placeholder read 3.54:1 in light against a token giving 4.88:1. Neither is visible to axe. That is the THIRD hand-written list to be wrong in this corner. | None | Move the 49 `.cm-*` rules into CodeMirror's own `EditorView.theme()`, where they participate in the same StyleModule and precedence as the defaults they override. Then the file can be layered like everything else. | Medium | No |
