...
| Tag vocabulary | `app/kernel/tags/` | The ONE tag normalisation rule (`normaliseTag`, `canonicalTagKey`), `WorkspaceTag`, validation, the bounds (`MAX_ENTITY_TAGS`, `MAX_TAG_LENGTH`, `TAG_VOCABULARY_READ_LIMIT`) and the read-only `TagVocabularyRepository`. Tags are WRITTEN by the record's own repository, so there is no tag write port. No D1/Cloudflare types. ([ADR-113](../decisions/ARCHITECTURE_DECISIONS.md#adr-113-a-tag-is-a-workspace-vocabulary-with-a-folded-key-and-an-owners-spelling--one-join-table-one-normalisation-rule-one-filter-dimension-and-a-tag-that-offers-rather-than-creates)) |
...
rather than an arbitrary JSON blob. The one structured field,
`navigation_config`, is versioned and validated by the preferences kernel because
module ids come from the registry. Reads with no row return deterministic code
defaults; no backfill is required.
