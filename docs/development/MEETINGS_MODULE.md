...
| The **same action is submitted concurrently** | Exactly one submission records; every other reports `already_held` with the ORIGINAL instant. One event, always. |

**Subject bound, disclosed.** FND-05 bounds one event at 32 subjects, leaving 31 attendees. A larger meeting records the earliest-linked 31, sets `attendeesTruncated` in the payload and reports both counts in the action's result — never a silent cap (AGENTS.md §6).
...
- **A sub-millisecond snapshot window.** Attendees are read immediately before the commit, so an attendee added inside that window may miss the snapshot. The held state itself is still exactly-once.
