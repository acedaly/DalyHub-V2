...
**The Activity payload stopped guessing.** `taskOutcome` used to be produced by
`catch { taskOutcome = "already_closed" }`, so a genuine storage failure, a
validation error and a Task the owner had actually already ticked were all recorded
as `already_closed` in the permanent event. It is now derived from a real pre-read:
`completed` when an open linked Task's statements are in this batch,
`already_closed` when the owner had already closed it (and no second
`task.completed` event is invented), `none` when no Task is linked. A rolled-back
completion appends no event at all.
...
one completed Task; two concurrent completions commit one of each; and another
workspace can neither complete the obligation nor touch its Task. See
