...
**The lifecycle journey fails intermittently on `main`** — `e2e/reviews.spec.ts:89` could not click *Restore review* on run 30306499933 because the shared notification stack intercepted the pointer event ([DEBT-38](../product/PRODUCT_DEBT.md#-debt-38--notification-toasts-occlude-bottom-anchored-record-actions--p1)). It **passed** on the next run against an identical test set, so it is a timing-dependent occlusion rather than a hard failure — a green run is not evidence it is fixed. It is one of several unrelated E2E failures currently keeping `main` red; see [DEBT-41](../product/PRODUCT_DEBT.md#-debt-41--the-e2e-suite-is-unreliable-on-main-so-ci-is-green-claims-are-unverifiable--p1) for the full picture.
...
- **Its Playwright journey fails intermittently on `main`.** `e2e/reviews.spec.ts:89` failed clicking *Restore review* because the shared DS-10 notification stack intercepted the pointer event; it passed on the following run with an identical test set. This is a shared-feedback-layer timing defect, not a Reviews modelling defect — [DEBT-38](../product/PRODUCT_DEBT.md#-debt-38--notification-toasts-occlude-bottom-anchored-record-actions--p1). It is one of several unrelated E2E failures keeping `main` red ([DEBT-41](../product/PRODUCT_DEBT.md#-debt-41--the-e2e-suite-is-unreliable-on-main-so-ci-is-green-claims-are-unverifiable--p1)).
