...
- **Authenticated composition (FND-09).** [FND-09](../roadmap/ROADMAP_V2.md#-fnd-09--app-shell-routing--auth) / [ADR-016](../decisions/ARCHITECTURE_DECISIONS.md#adr-016-cloudflare-access-identity-app-shell-and-registry-driven-routing) keeps the **same `WorkspaceContextResolver` interface** (workspace still server-derived) and adds `resolveAuthenticatedWorkspaceScope(env, session)`, which supplies the validated session's `sub` as the Activity `user` actor without changing module repository contracts. Authentication runs at the Worker request boundary before any protected loader/action.
- **Isolation is separate from authentication.** A `WorkspaceId` is a **scope identifier, not an authentication secret**. FND-03 resolves *which* scope from trusted server-side configuration; *who* the user is (Cloudflare Access, validated in the Worker) is FND-09 — the session never selects the workspace. See [`APP_SHELL_AUTH.md`](../development/APP_SHELL_AUTH.md).
...
- **Actor context.** Each event carries a trusted, server-derived `actor` (`{ type, id }`) established at the composition boundary and threaded into the mutation repositories — never accepted through a module method parameter. System-initiated composition uses the `system` actor (`id: null`); [FND-09](../roadmap/ROADMAP_V2.md#-fnd-09--app-shell-routing--auth) / [ADR-016](../decisions/ARCHITECTURE_DECISIONS.md#adr-016-cloudflare-access-identity-app-shell-and-registry-driven-routing) composes the validated Access session's `sub` into a `user` actor behind the same seam with no schema change.
...
## Shared frontend implementation layer

Current frontend implementation authority is [`UNTITLED_UI_IMPLEMENTATION.md`](../design/UNTITLED_UI_IMPLEMENTATION.md): Untitled UI React Pro source, Tailwind CSS v4 and React Aria. DalyHub-specific compositions remain in [`DESIGN_SYSTEM.md`](../design/DESIGN_SYSTEM.md). The older DS/DHDS descriptions below are historical compatibility records and do not authorize new generic primitives.
...
The foundation is now built ([ADR-017](../decisions/ARCHITECTURE_DECISIONS.md#adr-017-design-tokens-and-the-shared-record-layout)): the **design token system** ([DS-01](../roadmap/ROADMAP_V2.md#-ds-01--design-tokens--theming), [`app/styles/tokens.css`](../../app/styles/tokens.css) + [`app/shared/tokens`](../../app/shared/tokens)) is the authoritative, semantic, light/dark source of every visual value that CSS and components consume; and the entity-agnostic **Shared Record Layout** ([DS-02](../roadmap/ROADMAP_V2.md#-ds-02--shared-record-layout-header--summary--tabs), [`app/shared/record-layout`](../../app/shared/record-layout)) is the one header/summary/tabs/content scaffold every record view composes on those tokens. The entity-agnostic **Shared Drawer** ([DS-03](../roadmap/ROADMAP_V2.md#-ds-03--shared-drawer), [`app/shared/drawer`](../../app/shared/drawer), [ADR-018](../decisions/ARCHITECTURE_DECISIONS.md#adr-018-the-shared-drawer--url-driven-history-stacked-focus-isolated)) opens any record over the current page and hosts that Record Layout unchanged; its open stack is encoded in the URL (a repeated `drawer` query parameter) and integrated with browser history, so drawers are deep-linkable, stackable and Back/Forward-correct — the one shell-level effect is that `ScrollRestoration` is keyed to the pathname so query-only drawer/tab transitions preserve scroll. The entity-agnostic **Shared Card** ([DS-04](../roadmap/ROADMAP_V2.md#-ds-04--shared-cards), [`app/shared/card`](../../app/shared/card)) and **Shared Filters** ([DS-07](../roadmap/ROADMAP_V2.md#-ds-07--shared-filters), [`app/shared/filters`](../../app/shared/filters)) are now built together ([ADR-019](../decisions/ARCHITECTURE_DECISIONS.md#adr-019-shared-card-identity--reorder-and-the-filter-expression--url-contract)): one configurable Card renders every entity type in list/board/grid with selection, quick actions, density and an accessible pointer+keyboard reorder that emits intent, opening records through the DS-03 Drawer without owning drawer state; and one filter system supplies a typed, React-free expression model, a pure evaluator, a reusable Filter Bar with saved views, and a **repeated, versioned, safely-encoded filter URL contract** (`fv`/`f`/`fmode`) that composes with the Drawer's `drawer` parameter so a card can open over a filtered collection with filters preserved. Filter state is a first UI-architecture note beyond the shell: collection surfaces keep their filter state in the URL (never only in component state), the same way drawers do. No UI framework or drag-and-drop dependency was introduced (plain CSS custom properties + container queries + the browser platform). The Timeline/Activity-Feed, Forms, Search, Command-Palette and Inspector components remain later `DS-*` items.

The **Universal Relationship System** is the shared UI over the FND-04 EntityLink primitive ([ADR-047](../decisions/ARCHITECTURE_DECISIONS.md#adr-047-the-universal-relationship-system--one-shared-linked-items-surface-a-generic-links-endpoint-wiki-links-and-linked-boosting), [`RELATIONSHIPS.md`](../development/RELATIONSHIPS.md)): one reusable **Linked Items** section ([`app/shared/linked-items`](../../app/shared/linked-items)) every record's detail page mounts, backed by one authenticated **`/links`** endpoint (list/search/summary + link/unlink) so no module wires bespoke link routes. It adds inline `[[Wiki Links]]` (a pure transform in the FND-08 pipeline + a title resolver), hover-card summaries, and in-tier Search boosting of a record's directly-linked entities — all on the accepted primitives, with no second relationship model. Relationship create/remove flow through the existing shared Activity stream unchanged.

The **application frame** these components live in is now built ([PX-02](../roadmap/ROADMAP_V2.md#-px-02--product-frame), [ADR-020](../decisions/ARCHITECTURE_DECISIONS.md#adr-020-the-application-frame--sidebar-shell-pane-collection-layout-and-entity-identity)), realising the composition contract in [`PRODUCT_EXPERIENCE.md`](../design/UNTITLED_UI_MIGRATION.md#preserved-requirements-from-removed-design-records). FND-09's website-like top bar is replaced by a premium application shell ([`app/shared/shell`](../../app/shared/shell)): a persistent left **sidebar** (`grid-template-columns: var(--dh-shell-nav-width) 1fr`) owning identity, Search/Command entries, registry-driven **icon+label** navigation and a bottom **user menu** (identity + the relocated, unchanged theme control + settings + sign out), and a full-height **workspace pane** with its own sticky **Pane Header** over a document-scroll model that keeps the DS-03 Drawer's scroll contract intact. A one-icon **Entity Identity** system ([`app/shared/entity`](../../app/shared/entity)) pairs one icon + one accent token with each entity type (derived, per module, from its `entityTypes` manifest — no central switch), consumed by the sidebar, Pane Header, Cards and empty states. Three entity-agnostic screen scaffolds ship: a **Collection Layout** ([`app/shared/collection-layout`](../../app/shared/collection-layout)) — the named scaffold for the commonest screen, with built-in loading/empty/filtered-empty/error slots — one shared **EmptyState** ([`app/shared/empty-state`](../../app/shared/empty-state)) and a reduced-motion-aware **Skeleton** system ([`app/shared/skeleton`](../../app/shared/skeleton)); mobile navigation is an animated overlay sheet reusing the DS-03 focus machinery. The Tokens/Cards/Filters/Drawer/Record-Layout contracts are unchanged — the frame composes them. No new runtime dependency and no migration.
...
- **Client (settled — ADR-008 and ADR-125):** React 19 + TypeScript (strict), rendered through **React Router v8 in framework mode** (SSR by default), built with **Vite** and the official **Cloudflare Vite plugin**. Generic frontend implementation uses Untitled UI React Pro source with Tailwind CSS v4 and React Aria; see [`UNTITLED_UI_IMPLEMENTATION.md`](../design/UNTITLED_UI_IMPLEMENTATION.md).
...
- **Markdown & editor:** the shared, sanitising Markdown renderer is **accepted and built** ([ADR-006](../decisions/ARCHITECTURE_DECISIONS.md#adr-006-markdown-strategy) / [ADR-015](../decisions/ARCHITECTURE_DECISIONS.md#adr-015-markdown-source-and-safe-rendering-pipeline) / [FND-08](../roadmap/ROADMAP_V2.md#-fnd-08--markdown-pipeline)): Markdown source is the durable representation, rendered through one deterministic, Workers-safe `unified` pipeline with a strict allowlist and a single React sink ([`MARKDOWN_PIPELINE.md`](../development/MARKDOWN_PIPELINE.md)). A Markdown-native **editor** that still saves Markdown source is **built** for Notes ([NOTES-01B](../roadmap/ROADMAP_V2.md#-notes-01b--notes-collection-and-canonical-markdown-record)), with dependable autosave, a desktop Source/Split/Preview layout and soft-delete/restore lifecycle added by [NOTES-01C](../roadmap/ROADMAP_V2.md#-notes-01c--notes-autosave-lifecycle--editor-polish) — both reusing the same one sanitising pipeline, never a second parser or sink.
