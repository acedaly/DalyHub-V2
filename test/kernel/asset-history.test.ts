...
/**
 * V2.10 LIFE-01 — the ONE obligation store, wired the way the composition root
 * wires it. The Assets record's own behaviour is asserted through it, because
 * that is what the Assets record now reads.
 */
...
describe("asset lifecycle", () => {
  it("keeps history when the asset is archived, and restores it intact", async () => {
    ...
    const obligationRepo = obligations();
    const obligation = await obligationRepo.create({
...
describe("atomicity", () => {
  ...
  it("rolls a whole completion back — no event, no successor, no fact change", async () => {
    const asset = await ute();
    const setup = obligations();
    const obligation = await setup.create({
      subjectEntityId: asset.id,
      category: "service",
      title: "Service",
      dueDate: "2026-07-01",
    ...
    const faulty = obligations(WS, "f", { mutationFault: "after-domain" });
    await expect(
      faulty.complete(obligation.id, { completedOn: "2026-07-01" }),
    ).rejects.toBeTruthy();

    expect(await countAssetEventRows()).toBe(0);
    expect(await countObligationRows()).toBe(1);
    expect((await setup.get(obligation.id))?.status).toBe("open");
