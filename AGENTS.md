...
- **Branded Plum is the default visual identity.** Express it through Untitled's generated brand ramp: deep plum desktop/mobile navigation around a neutral light or dark working canvas, with purple reserved for primary, selected and focus interactions. Keep semantic success green, destructive/error red and warning amber. Do not create a separate palette system or use screenshots as theme authority; the complete rule lives in [`UNTITLED_UI_IMPLEMENTATION.md`](docs/design/UNTITLED_UI_IMPLEMENTATION.md#branded-plum).
...
- **The Untitled migration is COMPLETE, and that changes the shape of a frontend answer.** Every module has had its pass, and UNTITLED-19 audited the product for a competing generic system and closed the one it found. What remains is a ten-row register of named maintenance in [`UNTITLED_UI_MIGRATION.md`](docs/design/UNTITLED_UI_MIGRATION.md#named-maintenance-debt), each row a specific file or count, none of them blocking. "Migrate module X" and "one more phase" are no longer things to propose — a mature product has debt, and the register distinguishes migration debt from ordinary maintenance. Two rules carry most of the weight: a surface's stylesheet PAINT is deleted in the same change that migrates the surface (module CSS is unlayered, so a leftover rule silently outranks every Untitled utility and only a measurement finds it), and a `/design/*` fixture may only draw what the product draws. The next major initiative is architectural rather than visual — see [`DALYHUB_MOBILE_FOUNDATION.md`](docs/architecture/DALYHUB_MOBILE_FOUNDATION.md).
...
That is `markdown-editor-codemirror.css` — eight rules — and it is the only one. Keep such a file to the declarations the library itself sets, and DERIVE that set from the injected sheet rather than writing it down: `e2e/css-cascade-ownership.spec.ts` does, in both directions, because the two lists written by hand here were both wrong and both shipped. Un-layering the whole stylesheet that contains them takes the rest of the product's ability to compose with it away, silently. If you integrate another
...
- **A heading's anchor belongs to the PR that changes the heading.** Resolving or amending a [debt](docs/product/PRODUCT_DEBT.md) entry, or amending an [ADR](docs/decisions/ARCHITECTURE_DECISIONS.md) title, changes its heading and therefore its anchor — so **the PR that changes a heading repairs every link to it, in the same change.** Repair the *link*, never the heading: a heading carries its resolution on purpose, and renaming it back to resurrect an old anchor destroys the record to save a reference. `pnpm run docs:links:check` resolves every relative path and `#anchor` in `docs/**/*.md`, `AGENTS.md` and `README.md` under GitHub's slug rule, and **Static proves it on every change**. There is no allowlist and no annotation that excuses a finding; a missing screenshot is repaired by committing the image or by replacing the reference with a sentence saying what it showed (V2.9 INS-00, [DEBT-241](docs/product/PRODUCT_DEBT.md#-debt-241--no-documentation-link-or-anchor-check-exists-and-447-local-links-are-broken--p2--resolved-2026-09-04-v29-ins-00)).
