...
- **The design language is Material Design 3.** DalyHub does not invent a visual vocabulary. Colour, typography, shape, elevation, state and motion come from M3, hand-rolled in CSS over our own components — so "what radius does a chip take?" has a published answer rather than a DalyHub debate. Colour is *generated* from one seed and never authored; there is one light/dark pair and no theme feature. The owner's only choice about it is **Appearance** — System, Light or Dark — stored on their preference record and applied server-side to `<html data-appearance>`. See [`DESIGN_SYSTEM.md`](docs/design/DESIGN_SYSTEM.md), [ADR-074](docs/decisions/ARCHITECTURE_DECISIONS.md#adr-074-material-design-3-as-the-design-language--one-generated-scheme-no-theme-feature-and-an-alias-layer-as-the-migration-mechanism) and [ADR-075](docs/decisions/ARCHITECTURE_DECISIONS.md#adr-075-the-appearance-preference-and-one-authority-for-routine-creation).
