...
- **Respect user settings.** Honour `prefers-reduced-motion`, `prefers-color-scheme` and OS text scaling. **`System` is the default appearance, and it IS that honouring** — the device decides light or dark, and it decides for every colour scheme equally. An explicit **Light** or **Dark** is the owner overriding their device deliberately, which is user control rather than a failure to respect a device signal; the two are different things and only the second would be a defect. Every type size is authored in rem so OS text scaling reaches all of them. Motion never carries meaning that motion-off users lose.

  This bullet said `prefers-color-scheme` was *"the only thing that decides light or dark, because there is no theme preference to override it"* until **V2.16 CONSOL-03** corrected it — [DEBT-95](docs/product/PRODUCT_DEBT.md#-debt-95--agentsmd-6-and-15-still-describe-the-colour-system-the-product-no-longer-has--p2--closed-2026-09-08-v216-consol-03), raised 2026-08. It had been false since [ADR-075](docs/decisions/ARCHITECTURE_DECISIONS.md#adr-075-the-appearance-preference-and-one-authority-for-routine-creation) gave the owner a three-value Appearance preference, and it sat inside the accessibility requirements, which is the last place a reader should have to discount what they read. §6's half of the same entry had already been amended by DS-01; this was the half nobody had come back for.
