...
const COLLECTIONS: CollectionDescriptors = {
  ...
  /*
   * V2.12 FIN-00 — the Finance collections.
   *
   * There is NO balance column to read, on purpose: an archive carries the
   * opening balance and the transactions, and the figure is recomputed on the
   * way back in. The restore rehearsal asserts that every balance recomputes
   * IDENTICALLY, which a stored number could not promise (ADR-120 decision 5).
   *
   * A transaction's DISPLAY payee is the `entities` row's title and is carried
   * by the `entities` collection, so it is not repeated here.
   */
  financeAccounts: {
    ...
    map: (row) => ({
      entityId: requiredText(row.entity_id),
      accountType: requiredText(row.account_type),
      currencyCode: requiredText(row.currency_code),
      openingBalanceMinor: requiredInteger(row.opening_balance_minor, 0),
      openingDate: requiredText(row.opening_date),
  ...
  financeCategories: {
    ...
    map: (row) => ({
      ...
      nameKey: requiredText(row.name_key),
      kind: requiredText(row.kind),
      isBuiltin: requiredInteger(row.is_builtin, 0) === 1,
      sortOrder: requiredInteger(row.sort_order, 0),
      archivedAt: text(row.archived_at),
      createdAt: requiredText(row.created_at),
      updatedAt: requiredText(row.updated_at),
    }),
  ...
  financeImports: {
    ...
    map: (row) => ({
      ...
      fileSha256: requiredText(row.file_sha256),
      fileBytes: requiredInteger(row.file_bytes, 0),
      rowCount: requiredInteger(row.row_count, 0),
      addedCount: requiredInteger(row.added_count, 0),
      skippedExistingCount: requiredInteger(row.skipped_existing_count, 0),
      suspectedCount: requiredInteger(row.suspected_count, 0),
      invalidCount: requiredInteger(row.invalid_count, 0),
      mappingJson: requiredText(row.mapping_json),
  ...
  financeTransactions: {
    ...
    map: (row) => ({
      entityId: requiredText(row.entity_id),
      accountId: requiredText(row.account_id),
      occurredOn: requiredText(row.occurred_on),
      amountMinor: requiredInteger(row.amount_minor, 0),
      currencyCode: requiredText(row.currency_code),
      sourceDescription: requiredText(row.source_description),
      payeeKey: requiredText(row.payee_key),
      memo: text(row.memo),
      categoryId: text(row.category_id),
      categoryConfirmedAt: text(row.category_confirmed_at),
      importId: text(row.import_id),
      sourceTransactionId: text(row.source_transaction_id),
      fingerprint: requiredText(row.fingerprint),
