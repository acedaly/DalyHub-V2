...
// Source: `src/components/application/charts/charts-base.tsx`, retrieved 2026-09-12.
// Retrieved from the PUBLIC component API (`POST
// https://www.untitledui.com/react/api/components`, component `charts-base`)
// rather than from a licensed checkout, which this environment does not have.
// The API serves this component openly, not as PRO; it is exactly what
// `npx untitledui@latest add charts-base` installs, and it declares
// `recharts@^3.10.0` as its runtime.
...
// Listed in `API_SOURCED` in `scripts/vendor-untitled.mjs`, which therefore
// neither regenerates nor sweeps it. Only the import specifiers differ from
// upstream, plus:
//
// PATCHED by hand, and recorded in the script's `PATCHES` so a future re-vendor
// from a licensed checkout applies the same edit: upstream calls
// `payload.toReversed()`, an ES2023 array method. DalyHub's application
// tsconfig declares `lib: ["DOM", "DOM.Iterable", "ES2022"]`, so the call does
// not typecheck. `[...payload].reverse()` is the same non-mutating reversal at
// the library level the rest of the app compiles against.
"use client";
