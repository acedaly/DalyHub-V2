...
export function SegmentedFilter({
  ...
  return (
    <ViewSwitcher
      ...
      /*
       * RECORD-01 — a filter is SUBORDINATE to the tabs above it, and the
       * variant changes WEIGHT only: same anatomy, same one implementation,
       * same target, same keyboard behaviour. Applied here rather than per
       * caller because it follows from what a filter IS, so no module can
       * reintroduce a tab-weight filter by forgetting a prop.
       */
      weight="subtle"
    />
  );
