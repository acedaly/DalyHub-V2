...
/**
 * Habit — a closed loop with an arrowhead: a behaviour that comes round again.
 *
 * DalyHub's OWN drawing rather than a copied Material Symbol, authored in the
 * same 960-unit flipped space so it sits at the same optical weight as its
 * neighbours. It is deliberately NOT `check_circle` (which is the Task icon, and
 * a Habit is not a Task) and deliberately not `repeat` (which is Task
 * RECURRENCE, and conflating the two glyphs would undo the distinction the whole
 * module exists to hold). A loop is the honest metaphor: a practice returns, and
 * nothing about it is due.
 */
export const HabitIcon = createIcon(
  "HabitIcon",
  <>
    <path d="M645-766A330 330 0 1 1 315-766L345-714A270 270 0 1 0 615-714Z" />
    <path d="M628-812 748-740 628-668Z" />
  </>,
);

/**
 * Obligation — a clipboard with a clock: something committed to, with a time on
 * it. Material Symbols `pending_actions`.
 *
 * Deliberately NOT `check_circle` (the Task glyph — an obligation is not a
 * Task, and V2.10 exists to keep those apart), not `event_repeat` (the Review
 * glyph, and recurrence is a property of an obligation rather than its
 * identity), and not `receipt_long`, which would tie the identity to money when
 * an amount is optional and most obligations carry none.
 */
export const ObligationIcon = createIcon(
  "ObligationIcon",
