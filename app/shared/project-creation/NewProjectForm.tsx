/**
 * PROJ-01 / STEER-04 — the "New Project" form (hosted in the shared DS-03 Drawer).
 *
 * ── Why it lives in `app/shared` (STEER-04) ────────────────────────────────
 * It is now composed from a DIFFERENT module's record page — a Goal's — and the
 * cross-module import rule forbids `~/modules/goals` reaching into
 * `~/modules/projects` (`MODULES.md`, asserted by
 * `test/unit/module-registry/module-import-boundary.test.ts`). This is exactly
 * the move `NewGoalForm` made for the Area record, for the same reason, and the
 * point of making it rather than writing a second form is that there is still
 * ONE Project creation surface and ONE trusted endpoint behind it.
 *
 ...
 * rather than imported — importing a module's route or view types from
 * `app/shared` would itself be a module-boundary violation. This is the same
 * trade `NewGoalForm` records for `CreateGoalResult`, and the shapes are kept
 * in step by the Projects module's own route and component tests.
 */
...
interface NewProjectFormProps {
  ...
  /**
   * STEER-04 — the parent is DECIDED by the surface, so the picker is not shown.
   *
   * A Goal's record offers "New Project for this Goal"; there is exactly one
   * answer to "which Area or Goal?", and asking it again would be a form making
   * the owner re-find the record they are standing on — which is precisely the
   * research task DEBT-210 describes.
   *
   * It is a CONVENIENCE, never an authority: the id is posted as `parentId` to
   * the same `/projects/new` endpoint, and the server resolves the parent's KIND
   * from the id and re-verifies it belongs to this workspace, exactly as it does
   * for a picked one. A client cannot assert a Project's parentage here any more
   * than it can there.
   *
   * `label` is what the form SAYS the new Project will advance. It is display
   * text; nothing is resolved from it.
   */
  readonly fixedParent?: {
    readonly id: string;
...
export function NewProjectForm({
  ...
  const form = useForm<Values>({
    initialValues: {
      title: "",
      // STEER-04 — a fixed parent seeds the value the form posts, so the
      // required-field rule below is satisfied without a control the owner
      // would have nothing to choose in.
  ...
  /*
   * STEER-04 — with a fixed parent, neither empty-state branch below applies.
   *
   * Both are statements about the PICKER's option set: "we could not load your
   * Areas and Goals" and "this workspace has neither yet". A Goal's record has
   * already proven at least one exists — the owner is standing on it — so
   * showing either would be the form contradicting the page that opened it.
   */
  ...
  if (fixedParent === null && parentOptionsFailed) {
    return (
  ...
  if (fixedParent === null && parentOptions.length === 0) {
  ...
  return (
    ...
    >
      ...
      {templates.length > 0 ? (
        <SelectField
          ...
          onChange={(next) => {
            ...
            /*
             * STEER-04 — a template never moves a FIXED parent. The surface
             * decided which Goal this Project advances; a template's own
             * default parent is a suggestion for the picker, and there is no
             * picker here.
             */
            if (
              fixedParent === null &&
      ...
      {/*
       * STEER-04 — the parent, stated rather than chosen, when the surface
       * decided it. It is a sentence, not a disabled control: a greyed-out
       * picker invites the owner to try to change something they cannot, and a
       * disabled field is not reliably reachable by assistive tech. The value
       * itself rides in the form state and is re-verified server-side.
       */}
      {fixedParent ? (
        <p className="dh-form-fixed-parent" data-testid="new-project-parent">
          {`This Project will advance ${fixedParent.label}.`}
        </p>
      ) : (
        <SelectField
          label="Area or Goal"
